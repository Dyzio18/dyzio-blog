---
title: Redux Saga short-guide
date: '2023-10-12'
tags: ['redux', 'redux-saga', 'JavaScript', 'TypeScript']
draft: false
summary: Redux-Saga to narzędzie, które pomaga w zarządzaniu operacjami, które nie wykonują się natychmiast, takimi jak żądania do API. Działa w kontekście aplikacji opartych na Redux, umożliwiając tworzenie specjalnych 'wątków' - sag, które są odpowiedzialne za obsługę ciągów zdarzeń i odpowiedzi.
---
...
3. Tworzenie middleware dla sagi:
```
const sagaMiddleware = createSagaMiddleware();
```

4. Tworzenie store z middleware:
```
...
6. Tworzenie sagi (jako funkcji generatora):
```
function* mySaga() {
  yield takeEvery('FETCH_DATA', fetchData);
}
...
7. Obsługa asynchronicznych zadań:
```
function* fetchData() {
  try {
    const data = yield call(api.fetchData);
...
8. Przekazanie akcji z komponentu:
```
const dispatch = useDispatch();
